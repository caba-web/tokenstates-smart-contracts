...
contract Validator is Ownable, ReentrancyGuard {
        ...
        nonReentrant
    {
        ...
        _lockingToken.safeTransferFrom(msg.sender, address(this), _amount);
        ...
        if (_userTokens.initLocked == uint256(0)) {
            ...
            addOtherTokens(_tokenAddress, msg.sender, _amount);
        }
        tokens[_tokenAddress].wasSomethingLocked = true;
        emit Locked(_tokenAddress, msg.sender, _amount, block.timestamp);
    }
